...
function prompt(record) {
  ...
  const skillFolderPrerequisite = '<div class="skill-folder-prerequisite"><p><strong>One-Time Skills Folder Setup:</strong> Complete these steps once before you install your first Codex skill. You will use the same skills folder for every skill you install.</p><ol><li>Navigate to your user-profile directory:<ul><li>Windows: <code>C:\\Users\\&lt;username&gt;\\</code></li><li>Mac: <code>/Users/&lt;username&gt;/</code></li></ul></li><li>Create a folder named <code>.agents</code>.</li><li>Inside <code>.agents</code>, create a folder named <code>skills</code>. Each skill belongs in its own subfolder inside this <code>skills</code> folder.</li></ol></div>';
  ...
  const skillSection = skillAvailable ? '<section class="skill-install"><div class="skill-install-heading"><div><p class="eyebrow">Codex skill</p><h2>Download and install this skill</h2><p>A skill keeps this workflow available across future Codex work. Download the file, then place it in your local Codex skills folder.</p></div>' + skillDownloadControl + '</div>' + skillDetails + skillFolderPrerequisite + '<div class="skill-prerequisites">' + skillPrerequisites + prerequisiteLink + '</div><ol class="skill-install-steps"><li>Inside the skills folder above, create a subfolder:<ul class="skill-install-substeps"><li>' + skillFolderCopy + '</li></ul></li><li>Download <code>SKILL.md</code> and save it inside that subfolder as <code>SKILL.md</code>' + skillInlineDownloadControl + '.</li><li>Open a new Codex task and type:<ul class="skill-install-substeps"><li>' + skillInvocationCopy + '</li></ul></li><li><strong>Required Inputs</strong>' + requiredInputsInlineCopyControl + '<ol><li>Use the Copy button above to copy the <strong>Required Inputs</strong>, or review the Required Inputs section below.</li><li>Paste the completed inputs into the Codex chat below the skill name.</li><li>Enter values for your specific customer or use case after each equals sign.</li><li>Instruct Codex to execute the skill.</li></ol></li></ol><p id="download-skill-status" class="copy-prompt-status" aria-live="polite"></p></section>' : '<section class="skill-install skill-install-unavailable"><p class="eyebrow">Codex skill</p><h2>Skill download unavailable</h2><p>This legacy record has prompt text only. Its skill file has not been generated yet.</p></section>';
