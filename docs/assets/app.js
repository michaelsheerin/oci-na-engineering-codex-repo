...
function prompt(record) {
  ...
  const skillSection = skillAvailable ? '<section class="skill-install"><div class="skill-install-heading"><div><p class="eyebrow">Codex skill</p><h2>Download and install this skill</h2><p>A skill keeps this workflow available across future Codex work. Download the file, then place it in your local Codex skills folder.</p></div>' + skillDownloadControl + '</div>' + skillDetails + oneTimeSkillSetup() + '<div class="skill-prerequisites"><h3>Prompt prerequisites</h3>' + skillPrerequisites + prerequisiteLink + '</div><ol class="skill-install-steps"><li>Inside the skills folder above, create a subfolder:<ul class="skill-install-substeps"><li>' + skillFolderCopy + '</li></ul></li><li>Download <code>SKILL.md</code> and save it inside that subfolder as <code>SKILL.md</code>' + skillInlineDownloadControl + '.</li><li>Open a new Codex task and type:<ul class="skill-install-substeps"><li>' + skillInvocationCopy + '</li></ul></li><li><strong>Required Inputs</strong><ol><li>The skill prompts you for any Required Inputs needed.</li><li>Review the Required Inputs section below for the full list.</li></ol></li></ol><p id="download-skill-status" class="copy-prompt-status" aria-live="polite"></p></section>' : '<section class="skill-install skill-install-unavailable"><p class="eyebrow">Codex skill</p><h2>Skill download unavailable</h2><p>This legacy record has prompt text only. Its skill file has not been generated yet.</p></section>';
